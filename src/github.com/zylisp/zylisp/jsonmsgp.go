	...
	defer func() {
		...
		if recov != nil {
			...
			if ok {
				err := tc.TypeCheck()
				if err != nil {
					panic(fmt.Errorf("TypeCheck() error in zylisp.SexpToGo for '%T': '%v'", result, err))
				}
			}
		}
	}()
...
func SexpToGoStructs(
	...
	defer func() {
		...
		if recov != nil {
			...
			if ok {
				err := tc.TypeCheck()
				if err != nil {
					panic(fmt.Errorf("TypeCheck() error in zylisp.SexpToGoStructs for '%T': '%v'", result, err))
				}
			}
		}
	}()
	...
	case *SexpHash:
		...
		if targTyp.Kind() == reflect.Ptr && targTyp.Elem().Kind() == reflect.Interface && factType.Implements(targTyp.Elem()) {
			...
			// factType=*zylisp.NestInner/*reflect.rtype  targTyp = **zylisp.NestInner/*reflect.rtype
